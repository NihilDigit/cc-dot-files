...
派 agent 时不必显式指定模型。需要指定时，通用代码和复杂任务用 `opus`，小任务或持续性的简单任务用 `sonnet`。
...
删除、全局安装、破坏性 git 命令由 `~/.claude/hooks/` 的 hook 把关，被拒时按提示改用替代命令。以下几条 hook 管不到：

`*.<时间戳>.bak` 与仓库根的 `.claude-bak/` 是 hook 自动留下的备份，不要读取、提交或清理。

工作区存在未提交改动时，执行 reset、checkout、restore、clean、rebase 等会丢弃改动的 git 命令前，先说明将丢失哪些内容及原因。
